...
static void set_control_char (struct control_info const *info,
                              const char *arg,
                              struct termios *mode);
static void set_speed (const char *device_name, enum speed_setting type, const char *arg,
                       struct termios *mode);
      ...
      if (!match_found || not_set_attr)
        {
          if (STREQ (arg, "ispeed"))
            {
              set_speed (device_name, input_speed, settings[k], mode);
              *speed_was_set = true;
              *require_set_attr = true;
            }
          else if (STREQ (arg, "ospeed"))
            {
              set_speed (device_name, output_speed, settings[k], mode);
              *speed_was_set = true;
              *require_set_attr = true;
            }
#ifdef TIOCEXT
          /* This is the BSD interface to "extproc".
            ...
            {
              set_speed (device_name, both_speeds, arg, mode);
              *speed_was_set = true;
              *require_set_attr = true;
            }
          else if (string_to_baud (arg) == (speed_t) -1)
...
static void
set_speed (const char *device_name, enum speed_setting type, const char *arg, struct termios *mode)
{
  speed_t baud;
 int fd = open(device_name, O_RDONLY);
 (void)mode;
  baud = string_to_baud (arg);
  struct termios2 mode2;
/*  if (type == input_speed || type == both_speeds)
    cfsetispeed (mode, baud);
  if (type == output_speed || type == both_speeds)
    cfsetospeed (mode, baud);
    */
    ioctl(fd, TCGETS2, &mode2);
    mode2.c_cflag &= ~CBAUD;
    mode2.c_cflag |= BOTHER;
    mode2.c_ispeed = baud;
    mode2.c_ospeed = baud;
    int r = ioctl(fd, TCSETS2, &mode2);
}
